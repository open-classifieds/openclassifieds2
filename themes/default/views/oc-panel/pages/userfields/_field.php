<li data-id="<?=$name?>" id="li_<?=$name?>">
    <a href="<?=Route::url('oc-panel',array('controller'=>'userfields','action'=>'update','id'=>$name))?>" class="block hover:bg-gray-50 focus:outline-none focus:bg-gray-50 transition duration-150 ease-in-out">
                    ...
                    <div>
                        <div class="text-sm leading-5 text-gray-900 truncate">
                            ...
                            <? if($field['show_profile']) : ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-gray-100 text-gray-800"><?=(isset($field['show_profile']) AND $field['show_profile'])?__('Show profile'):NULL?></span>
                            <? endif ?>
                            <? if($field['show_register']) : ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-gray-100 text-gray-800"><?=(isset($field['show_register']) AND $field['show_register'])?__('Show register'):NULL?></span>
                            <? endif ?>
